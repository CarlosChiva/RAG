...
async def registrer_token(user_name: str, token: str):
    ...
    mysql_cursor = db.cursor()
    mysql_cursor.execute(
        "UPDATE users SET jwt_token = %s  WHERE username = %s ;", (token, user_name)
    )
    mysql_cursor.close()
    db.commit()
    db.close()
...
async def check_user_by_credential(credential: str):
    ...
    mysql_cursor = db.cursor()
    mysql_cursor.execute(
        "SELECT id_user FROM users WHERE jwt_token = %s ;", (credential,)
    )
...
async def get_user_services(id_user: str):
    try:
        ...
        # Execute the query
        mysql_cursor.execute(
            "SELECT chatbot, pdf, multimedia, excel,ddbb FROM services WHERE user_id = %s",
            (id_user,),
        )
...
async def add_user_services(service: str, id_user: str):
    # Whitelist of valid services
    valid_services = ["chatbot", "pdf", "multimedia", "excel", "ddbb"]
    if service not in valid_services:
        return f"Error: Invalid service '{service}'. Valid services are: {', '.join(valid_services)}"

    db = await db_connect()
    mysql_cursor = db.cursor()
    query = "UPDATE services SET {} = TRUE WHERE user_id = {} ;".format(
        service, id_user
    )
...
async def remove_user_services(service: str, id_user: str):
    ...
    db = await db_connect()
    mysql_cursor = db.cursor()
    query = " UPDATE services SET {} = FALSE WHERE user_id = {} ;".format(
        service, id_user
    )
